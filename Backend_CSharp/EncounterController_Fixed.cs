...
public class EncounterController : ControllerBase
{
    ...
    /* --------- Lịch sử khám bệnh (phân trang) - ĐÃ SỬA --------- */
    [HttpGet]
        ...
        CancellationToken ct = default)
    {
        ...
        // SỬA: Join với Appointment để lấy status từ appointment nếu encounter vẫn là draft
        var items = await q
            ...
            {
                Encounter = e,
                // Tìm appointment tương ứng (match theo patient, doctor, và ngày gần nhất)
                Appointment = _db.Appointments.AsNoTracking()
                    .Where(a => a.PatientId == e.PatientId 
                        && a.DoctorId == e.DoctorId
                        && a.Status == "done"
                        && (a.VisitDate.Date == e.CreatedAt.Date || 
                            Math.Abs((a.VisitDate.Date - e.CreatedAt.Date).Days) <= 1))
        ...
        {
            var e = x.Encounter;
            // Nếu encounter là draft và có appointment với status "done", thì dùng status "finalized"
            var finalStatus = e.Status?.ToLower() == "draft" && x.Appointment != null && x.Appointment.Status == "done"
                ? "finalized"
                : e.Status ?? "draft";
    ...
    public async Task<IActionResult> GetEncounterDetail([FromRoute] string codeOrId, CancellationToken ct = default)
    {
        ...
        // Kiểm tra appointment tương ứng
        var appointment = await _db.Appointments.AsNoTracking()
            .Where(a => a.PatientId == encounter.PatientId
                && a.DoctorId == encounter.DoctorId
                && a.Status == "done"
                && (a.VisitDate.Date == encounter.CreatedAt.Date ||
                    Math.Abs((a.VisitDate.Date - encounter.CreatedAt.Date).Days) <= 1))
            .OrderByDescending(a => a.VisitDate)
            .FirstOrDefaultAsync(ct);

        // Nếu encounter là draft và có appointment done, dùng status finalized
        var finalStatus = encounter.Status?.ToLower() == "draft" && appointment != null && appointment.Status == "done"
            ? "finalized"
